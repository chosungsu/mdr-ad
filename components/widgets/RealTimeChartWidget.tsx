...
const RealTimeChartWidget: React.FC<RealTimeChartWidgetProps> = ({ isRunning }) => {
  ...
  // Update chart when new dashboard data arrives (every 2 seconds)
  useEffect(() => {
    ...
    const newPoint: DualMetricPoint = {
      ...
      mdrad: dashboardData.scores.mdrad ?? 0,
    };

    setData(prevData => {
      const newData = [...prevData, newPoint];
      if (newData.length > 60) newData.shift(); // Keep last 60 points (2 minutes at 2s interval)
      return newData;
    });
