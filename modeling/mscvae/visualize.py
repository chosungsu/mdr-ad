...
def visualize_anomalies(
    ...
    ckpt_latent_dim = checkpoint.get("latent_dim", latent_dim)
    ckpt_lstm_hidden = checkpoint.get("lstm_hidden", lstm_hidden)
    ...
    ax_score.set_title("MSCVAE Anomaly Detection Scores", fontsize=14, fontweight="bold")
    ...
    plt.suptitle("MSCVAE Anomaly Detection Visualization", fontsize=16, fontweight="bold", y=0.995)
    ...
    ax.set_title("MSCVAE Anomaly Score (75%/95% 기준선 및 범주)", fontsize=12, fontweight="bold")
...
def main():
    parser = argparse.ArgumentParser(description="Visualize MSCVAE anomaly detection results")
    parser.add_argument(
        "--data-path",
        type=str,
