...
def train(model, dataloader, epochs: int, device: torch.device):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x_batch in dataloader:
            ...
            loss = tcad_loss(x_batch, x_hat, z1, z2)
            loss.backward()
            optimizer.step()
...
def main():
    parser = argparse.ArgumentParser(description="Train TCAD model on sensor_data_rms2_fixed.csv")
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        "--model-path", type=str, default="tcad_model.pt", help="학습된 모델 저장 경로"
    )
